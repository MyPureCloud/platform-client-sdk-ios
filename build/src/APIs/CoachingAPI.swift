...
open class CoachingAPI {
  ...
  "results" : [ {
    ...
    /**
     ...
     - parameter completion: completion handler to receive the data and the error objects
     */
    open class func postCoachingScheduleslotsQuery(body: CoachingSlotsRequest, completion: @escaping ((_ data: CoachingSlotsResponse?,_ error: Error?) -> Void)) {
        let requestBuilder = postCoachingScheduleslotsQueryWithRequestBuilder(body: body)
        requestBuilder.execute { (response: Response<CoachingSlotsResponse>?, error) -> Void in
            do {
                if let e = error {
                    completion(nil, e)
                } else if let r = response {
